---
name: setup-competitor-watch
description: >
  First-time setup and reconfiguration skill for the competitor-watch use case.
...
**Writes to:** `/agent/brain/competitor-watch/{{WORKSPACE_SLUG}}/watchlist.json`
...
Check if `/agent/brain/competitor-watch/{{WORKSPACE_SLUG}}/watchlist.json` already exists.
...
Once brands are confirmed, write to `/agent/brain/competitor-watch/{{WORKSPACE_SLUG}}/watchlist.json`:
...
If yes, immediately invoke the `competitor-watch` skill.
