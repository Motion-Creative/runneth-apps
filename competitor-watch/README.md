...
|------|-------------|-----------|
| `SKILL.md` | `/agent/.agents/skills/competitor-watch/SKILL.md` | Overwrite |
| `setup-competitor-watch/SKILL.md` | `/agent/.agents/skills/setup-competitor-watch/SKILL.md` | Overwrite |
| `seed/watchlist.json` | `/agent/brain/competitor-watch/{{WORKSPACE_SLUG}}/watchlist.json` | Skip |
| `seed/baselines/.gitkeep` | `/agent/brain/competitor-watch/{{WORKSPACE_SLUG}}/baselines/.gitkeep` | Skip |

Post-install: the `setup-competitor-watch` skill runs automatically to configure the watchlist,
...
mkdir -p /agent/.agents/skills/competitor-watch
cp SKILL.md /agent/.agents/skills/competitor-watch/SKILL.md

mkdir -p /agent/.agents/skills/setup-competitor-watch
cp setup-competitor-watch/SKILL.md /agent/.agents/skills/setup-competitor-watch/SKILL.md

# 2. Seed the brain directory (replace {{WORKSPACE_SLUG}} with the actual slug)
WORKSPACE_SLUG="your-workspace-slug"
mkdir -p /agent/brain/competitor-watch/${WORKSPACE_SLUG}/baselines
[ -f /agent/brain/competitor-watch/${WORKSPACE_SLUG}/watchlist.json ] || \
  cp seed/watchlist.json /agent/brain/competitor-watch/${WORKSPACE_SLUG}/watchlist.json
...
**Setup (one time):** The `setup-competitor-watch` skill walks through:
...
```
/agent/brain/competitor-watch/{{WORKSPACE_SLUG}}/
...
All tokens are set automatically by the `setup-competitor-watch` skill.
...
| "set up competitor watch" | Invokes `setup-competitor-watch` for add/remove/reconfigure |
| "add a competitor" | Invokes `setup-competitor-watch` to add one brand |
| "remove a competitor" | Invokes `setup-competitor-watch` to remove one brand |
...
1. Reads `/agent/brain/competitor-watch/{{WORKSPACE_SLUG}}/watchlist.json` for brand IDs and config
...
7. Loads baseline from `/agent/brain/competitor-watch/{{WORKSPACE_SLUG}}/baselines/{slug}.json`
...
- **1.0.0** (2026-05-20) — Initial release. Translated from the `competitor-watch` skill in
