...
Read `/agent/brain/competitor-watch/{{WORKSPACE_SLUG}}/watchlist.json`.

If the file is missing or `brands` is empty, halt and invoke `setup-competitor-watch`:
...
`/agent/brain/competitor-watch/{{WORKSPACE_SLUG}}/baselines/{brand-slug}.json`

Track which brands have an existing baseline (delta run) vs which are first runs (baseline-only).

### 1e. Load Slack config

Read `slackChannelId` from `/agent/brain/competitor-watch/{{WORKSPACE_SLUG}}/watchlist.json`.
...
**Baseline file path:** `/agent/brain/competitor-watch/{{WORKSPACE_SLUG}}/baselines/{brand-slug}.json`
...
| Condition | Response |
|-----------|----------|
| Watchlist missing or empty | Invoke `setup-competitor-watch`, then retry |
