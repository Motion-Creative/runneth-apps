...
const BRAIN_STRUCTURE_BY_KEY: Record<string, string> = {
  ...
  connected_systems: 'Free-text only, no files. Capture the list in `/agent/brain/customers/<slug>/summary.md` under a `### Systems and integrations` section so it surfaces on every future turn. For each named system, note what lives there (briefs, customer reviews, assets, project tracking, decisions). Pay special attention to review platforms (Trustpilot, Yotpo, Shopify, Amazon, support tools) — customer reviews are no longer collected via file upload, so the review-platform connection is the primary path for VOC. If any are first-class Runneth integrations (Notion, Google Drive, Slack, HubSpot, Linear, Intercom) or Pipedream-connected apps (Airtable, Asana, Monday, ClickUp, Figma, Dropbox, review platforms), open a follow-up to walk the customer through `oauth-connect` for each one on their first session.',
