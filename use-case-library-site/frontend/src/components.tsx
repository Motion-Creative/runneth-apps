...
export const ExperimentalPill = ({ pulse }: { pulse?: boolean }): JSX.Element => (
  ...
  >
    <span style={{ width: 6, height: 6, borderRadius: 999, background: colors.experimentalText }} />
    Experimental
  </span>
)

export const CategoryPill = ({ title, accentHex }: { title: string; accentHex: string }): JSX.Element => (
  <span
    style={{
      display: 'inline-flex',
      alignItems: 'center',
...
export const UseCaseCard = ({ useCase, index }: { useCase: UseCaseMeta; index: number }): JSX.Element => {
  ...
  return (
    <motion.div
      ...
      whileHover={{ y: -4, boxShadow: `0 18px 40px ${accentHex}26, 0 4px 10px rgba(0,0,0,0.04)` }}
