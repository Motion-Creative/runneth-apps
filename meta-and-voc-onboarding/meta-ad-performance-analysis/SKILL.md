...
- **Read customer voice for customer-side WHY.** When the question asks why customers
  ...
  read `/agent/brain/brands/<brand>/customer-feedback/voice-of-customer-audit.md` when it exists. Use its
  cited raw files under `/agent/brain/brands/<brand>/customer-feedback/<platform>/` to verify or deepen a
...
- **Account context.** `/agent/brain/brands/<brand>/integrations/meta/account-context.md` is read first (per the guard above) and defines how this account judges performance; if it names the KPI, that is the answer.
...
7. To explain any of it, read the creative itself — and the customer. Metrics locate where
   ...
   Audit (`/agent/brain/brands/<brand>/customer-feedback/voice-of-customer-audit.md`) first,
