...
- **Read customer voice for customer-side WHY.** When the question asks why customers
  ...
  read `/agent/brain/data-sources/voc/voice-of-customer-audit.md` when it exists. Use its
  cited raw files under `/agent/brain/data-sources/voc/<platform>/` to verify or deepen a
...
7. To explain any of it, read the creative itself — and the customer. Metrics locate where
   ...
   Audit first, then its cited reviews, support themes, community posts, and ad comments
   when the question is how people are responding. Tie the drop-off to what
