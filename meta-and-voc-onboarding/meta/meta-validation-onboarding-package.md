...
**Account Context Brain** (`/agent/brain/brands/<brand>/integrations/meta/account-context.md`) and the
...
`/agent/brain/brands/<brand>/integrations/meta/account-context.md` (and its satellite, the naming decoder — Field 10 is one of its ten fields) —
...
`/agent/brain/installed-packages/meta-and-voc-onboarding/meta-onboarding-rules/meta-analysis-validation.md` - merge from that file,
...
- Brand folder: `/agent/brain/brands/<brand>/`, where `<brand>` is this conversation's
  ...
  conversation's brand folder, and each workspace validates independently.
- When the Account Context Brain (/agent/brain/brands/<brand>/integrations/meta/account-context.md) has all required
  ...
  completed (/agent/brain/brands/<brand>/integrations/meta/validation.md missing or MVCE state = off), open the validation
...
- Validation is complete only when: must-have Meta context sources are connected and refreshing,
  ...
  /agent/brain/brands/<brand>/integrations/meta/validation.md.
...
1. **Account Context Brain is `[CONFIRMED]`** - check the field statuses in the "File
   metadata" block at the end of `/agent/brain/brands/<brand>/integrations/meta/account-context.md`. The interpretation
...
Save the validation record to `/agent/brain/brands/<brand>/integrations/meta/validation.md`. It captures:
...
5. Show me all our [product] ads — using a real product or concept name from the confirmed
   naming decoder.
6. When `/agent/brain/brands/<brand>/customer-feedback/voice-of-customer-audit.md` exists: What are
...
- Register is a correctable read too. "I just wanted the table" or "why didn't you explain
  ...
  posture.
- Every correction is logged to `/agent/brain/brands/<brand>/integrations/meta/validation.md` and applied to
  `/agent/brain/brands/<brand>/integrations/meta/account-context.md` so the fix is durable.
...
Record the state in `/agent/brain/brands/<brand>/integrations/meta/validation.md`:
...
- Log re-validations in
  `/agent/brain/brands/<brand>/integrations/meta/_changelog.md`, the same convention the other
  Meta parts use.
