...
**Account Context Brain** (`/agent/brain/<workspace>/account-context.md`) and the
...
`/agent/brain/<workspace>/account-context.md` (and its satellites: the naming decoder, Field 10) —
...
- When the Account Context Brain (/agent/brain/<workspace>/account-context.md) has all required
  ...
  (/agent/brain/<workspace>/validation.md missing or MVCE state = off), open the validation
...
- Validation is complete only when: must-have Meta context sources are connected and refreshing,
  ...
  /agent/brain/<workspace>/validation.md.
...
1. **Account Context Brain is `[CONFIRMED]`** - check the fields-confirmed count in the "File
   metadata" block at the end of `/agent/brain/<workspace>/account-context.md`. All nine required interpretation fields signed off
...
Save the validation record to `/agent/brain/<workspace>/validation.md`. It captures:
...
- Register is a correctable read too. "I just wanted the table" or "why didn't you explain
  ...
  posture.
- Every correction is logged to `/agent/brain/<workspace>/validation.md` and applied to
  `/agent/brain/<workspace>/account-context.md` so the fix is durable.
...
Record the state in `/agent/brain/<workspace>/validation.md`:
...
- Log re-validations in `/agent/brain/<workspace>/_changelog.md`, the same convention the other parts use.
