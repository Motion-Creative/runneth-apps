...
- **WHY — creative content and customer voice.** Two why-sources, two sides of the same
  ...
  - **VoC is the customer side:** why people respond the way they do. Read
    `/agent/brain/<brand>/integrations/voice-of-customer/voice-of-customer-audit.md` first when it exists, then
    inspect the raw reviews, support conversations, community posts, and ad comments it
    cites under `/agent/brain/<brand>/integrations/voice-of-customer/<platform>/`. If raw VoC exists but the
...
(`/agent/brain/<brand>/integrations/meta/naming-decoder.json`, owned by Account Context Field 4) and present the
