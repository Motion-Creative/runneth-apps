...
- **The package installs automatically only for matching intent, but installing does not act.** The activation discloses the account reads and persistent setup, and an explicit human yes is required before the combined run starts. Existing orgs can still get a separately approved cleanup pass.
