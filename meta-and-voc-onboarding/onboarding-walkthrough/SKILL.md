...
`/agent/brain/installed-packages/meta-and-voc-onboarding/meta-account-context-brain-onboarding-package.md` (the ACB
package). Read that document before presenting; this skill is the how-to-say-it, that
document is the what-it-means.

## Pre-flight

1. Read `/agent/brain/brands/<brand>/integrations/meta/account-context.md`. Post-install writes it as a scaffold: every
...
- Handle answers and corrections per the ACB package's field rules, and persist them to
  `/agent/brain/brands/<brand>/integrations/meta/account-context.md` as they confirm.
...
- **Then, with the Meta beats done, present the Voice of Customer summary — proactively, not
  ...
  `/agent/brain/brands/<brand>/customer-feedback/` and its `voc-sync-<brand>-*` routines,
...
- **Then offer the Voice of Customer Audit by previewing the plan, in Runneth's own words —
  ...
  - **Data ready, no audit yet:** check whether
    `/agent/brain/brands/<brand>/customer-feedback/voice-of-customer-audit.md` exists; if not,
    make the offer above. If `/agent/brain/brands/<brand>/_changelog.md` has no
