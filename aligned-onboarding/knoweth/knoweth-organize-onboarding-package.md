...
**MERGE INSTRUCTIONS:** For each block, if its sentinel (`runneth:knoweth-organize` or `runneth:knoweth-brain`) already exists in `/agent/user.md`, replace it in place. Otherwise append it. Never duplicate. Do not edit anything outside the sentinels. The canonical copies of these blocks are the staged guard files `/agent/brain/aligned-onboarding/guards/knoweth-organize.md` and `/agent/brain/aligned-onboarding/guards/knoweth-brain.md` - merge from those files (the package's post-install run does this in its single guard merge); the blocks below are shown for context and must stay identical to the staged files.
