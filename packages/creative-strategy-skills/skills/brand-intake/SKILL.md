...
#### 4. Known Competitors
- Who are the 3–5 closest competitors? (Use what the user told you in Phase 1 as a starting point)
- If the user didn't name any, do a quick search — `"[brand name] vs"` and `"[product category] alternatives"` — to surface the most commonly mentioned names.
- **Do not deep-profile competitors here.** Just capture the names and a one-line description of what each one is. Deep competitive research belongs in the `competitor-analysis` skill.
