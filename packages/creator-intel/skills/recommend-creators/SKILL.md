...
- For each seed, read the profile and the accounts it follows. Evaluate every followed account against the target's niche/topic fit, using the same content-beats-profile filter method A uses. Keep all that pass the niche filter as candidates; rank them by how many seeds' networks they appear in. Dedupe against the roster and against each other.
- Run once, bounded to the top 6 to 10 seed profiles when the customer has that many good sources available, and about 25 ranked candidates. More seeds means more raw followed-accounts to filter for niche fit, which is what produces usable candidates once the overlap requirement is removed. Resolve any candidate that also exists in Motion with `motion inspo creators`; keep off-Motion candidates as raw handles with their profile link.
- This path needs an Apify key, used through `secure-fetch` against `api.apify.com`. Working recipe, verified live:
  ...
  - Seeds: the customer's best creators for the target plus their named north-stars. Run each seed, then evaluate every followed account against the target's niche/topic fit. Keep all that pass the niche filter as candidates; rank them by how many seeds' networks they appear in. Exclude the seeds themselves and anyone already on the roster (including the customer's own team handles).
...
- Enrich every candidate before presenting: run the profile scraper (see `creator-intel-reference/apify-actors.md`) to get follower count and bio. Present at least 6 to 10 creators, each with: the Instagram link (`https://www.instagram.com/<handle>/`), the follower count, what they talk about, and the reason you picked them: niche/topic fit against the target (evaluated the same way as method A), with a priority boost when 2 or more seeds follow them. Apply judgment: drop off-fit accounts (lifestyle, unrelated niches, vague mega-accounts) at this stage, not after, and say you filtered them.
