...
- The voc-onboarding-walkthrough skill (the voc-onboarding package's entry point, fired by
  a person asking to run their Voice of Customer onboarding) is executing its setup phase -
  that ask is the trigger - or the team or user asks to set up the VoC data sync -> run
  the "Set up the recurring sync" procedure below. Setup never happens at any other
  unprompted moment - never at install, never just because a platform connects.
