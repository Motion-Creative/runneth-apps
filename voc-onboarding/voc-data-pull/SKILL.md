...
- The voc-onboarding activation disclosed the setup and received an explicit
...
- **The later audit is not a pull output.** The Voice of Customer audit skill runs later,
  after raw data has landed. It writes
...
batched shell loop of `integrations status --app <slug>` over the slugs in the table
above - a single command whose output is the whole OAuth inventory, never separate
per-slug commands, never speculative slugs beyond the table, and never an
`integrations list` probe, which reports catalog metadata only, not connection state)
**and the runtime secret store's key names with
