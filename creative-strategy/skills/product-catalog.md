...
Fetch the page and extract all product URLs. Look for URL patterns like `/products/`, `/product/`, `/p/`, `/item/`, or `/collections/[name]/products/`.
...
For each product URL, fetch the page and extract only what is explicitly stated on it. Do not infer, editorialize, or add context.
