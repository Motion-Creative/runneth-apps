...
type SqliteInfo = Readonly<{ path: string; version: string }>
...
const resolveSqliteVersion = (): string => {
  ...
  if (!row || typeof row !== 'object') { throw new Error('Invalid SQLite version response') }
  const value = row['version']
  if (typeof value !== 'string' || value.length === 0) { throw new Error('SQLite version value is missing') }
  return value
}

export const getSqliteInfo = (): SqliteInfo => {
  return { path: resolveSqlitePath(), version: resolveSqliteVersion() }
}
