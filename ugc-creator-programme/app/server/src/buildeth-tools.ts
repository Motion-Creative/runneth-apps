...
const resolveRuntimeToolsBundlePath = (): string => {
  ...
  if (!configuredPath) {
    return DEFAULT_RUNTIME_TOOLS_BUNDLE_PATH
  }
...
const toRuntimeToolsContext = (runtime: BuildethRuntimeContext): BuildethRuntimeToolsContext => {
  return {
    ...
    auth: {
      get token(): string {
        return requireRuntimeValue(
          runtime.authToken,
          'Buildeth runtime backend auth is required for runtime tools'
        )
      },
    },
    get backendApiUrl(): string {
      return requireRuntimeValue(
        runtime.backendApiUrl,
        'Buildeth runtime backend auth is required for runtime tools'
      )
    },
    ...
    get workspaceId(): string {
      return requireRuntimeValue(
        runtime.workspaceId,
        'Buildeth runtime workspaceId is required for runtime tools'
      )
    },
  }
}
