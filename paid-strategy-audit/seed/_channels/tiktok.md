...
- Read `motion workspace-goal` as a hint (it defaults to ROAS; a non-ROAS value is a deliberate signal), not the KPI. Confirm `motion spend-threshold` with the user and always judge creatives on spend + KPI together — a creative below threshold is "not enough data," not a winner or loser. TikTok's faster creative churn often warrants a different threshold than Meta in the same account.
